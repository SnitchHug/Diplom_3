...
class TestMainPage:
    ...
    @allure.story('Открытие всплывающего окна с деталями ингредиента')
    ...
    @allure.feature('Функционал сборки заказа')
    @allure.story('При добавлении ингредиента в заказ, увеличивается счетчика данного ингредиента')
    @allure.title('Тест на проверку увеличения счетчика у ингредиента который добавили в заказ')
    ...
    def test_place_order_as_logged_user(self, main_page, login_in):
        ...
        assert main_page.wait_for_popup_order_id_window() and order_id is not None and order_id != 9999, \
            ("Не открылось всплывающее окно с идентификатор заказа, "
