...
class BasePage:
    ...
    def is_element_absent(self, locator, timeout=20):
        try:
    ...
    def drag_and_drop_on_to_element(self, source_locator, target_locator):
        ...
        self.driver.execute_script(from_element, to_element)

    # Ожидает, пока текст элемента не изменится с initial_text на любой другой.
    def wait_for_text_to_change(self, locator, initial_text, timeout=20):
    ...
    def wait_for_text_to_be(self, locator, expected_text, timeout=20):
        WebDriverWait(self.driver, timeout).until(
